...
async function askMaxielAI(question) {
  const prompt = `
Kamu adalah Maxiel AI.

Kamu adalah asisten virtual resmi dari website Maxiel Nuoye.

Aturan:
- Selalu menjawab dalam bahasa Indonesia kecuali pengguna meminta bahasa lain.
...
`
  ...
  const response = await fetch(API.publicAI(prompt), {
    method: "GET",
    headers: {
      "Accept": "text/plain"
    }
