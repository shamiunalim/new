...
}\n\n// =========================\n// WALLET / TOP UP / TRANSFER / TABUNG (GITHUB PAGES LOCAL)\n// =========================\n(() => {\n  const KEY = "maxiel_wallet_local_v2"\n  const defaults = { balance: 0, savings: 0, history: [] }\n  const money = n => "Rp " + Number(n || 0).toLocaleString("id-ID")\n  function load(){ try { return {...defaults,...(JSON.parse(localStorage.getItem(KEY)) || {})} } catch { return {...defaults} } }\n  let w = load()\n  function persist(){ localStorage.setItem(KEY, JSON.stringify(w)); render() }\n  function add(type, amount, detail){ w.history.unshift({type,amount,detail,time:new Date().toLocaleString("id-ID")}); w.history=w.history.slice(0,100) }\n  function render(){\n    const b=document.getElementById("walletBalance"), s=document.getElementById("savingBalance"), h=document.getElementById("walletHistory")\n    if(b) b.textContent=money(w.balance)\n    if(s) s.textContent=money(w.savings)\n    if(h) h.innerHTML=w.history.length ? w.history.map(x=>`<div class="wallet-history-item"><div><b>${safe(x.detail)}</b><br><span>${safe(x.time)}</span></div><b>${x.type==="plus"?"+":"-"}${money(x.amount)}</b></div>`).join("") : '<div class="wallet-empty">Belum ada transaksi.</div>'\n  }\n  function safe(v){ const d=document.createElement("div"); d.textContent=v ?? ""; return d.innerHTML }\n  function openPanel(name){ document.querySelectorAll(".wallet-panel").forEach(x=>x.classList.remove("show")); const el=document.getElementById("wallet"+name+"Panel"); if(el) el.classList.add("show") }\n  document.addEventListener("click", e=>{\n    const action=e.target.closest("[data-wallet-action]")\n    if(action){ openPage("wallet"); const a=action.dataset.walletAction; openPanel(a==="topup"?"Topup":a==="transfer"?"Transfer":a==="save"?"Save":"History"); return }\n    const close=e.target.closest("[data-wallet-close]")\n    if(close){ document.getElementById("wallet"+close.dataset.walletClose.charAt(0).toUpperCase()+close.dataset.walletClose.slice(1)+"Panel")?.classList.remove("show") }\n  })\n  document.addEventListener("click", e=>{\n    if(e.target.id==="topupBtn"){ const a=Number(document.getElementById("topupAmount").value); if(!Number.isFinite(a)||a<1000)return toast("Minimal top up Rp 1.000."); w.balance+=a; add("plus",a,"Top Up"); document.getElementById("topupAmount").value=""; persist(); toast("Top up berhasil") }\n    if(e.target.id==="transferBtn"){ const to=document.getElementById("transferTo").value.trim(),a=Number(document.getElementById("transferAmount").value); if(!to)return toast("Masukkan penerima."); if(!Number.isFinite(a)||a<1000)return toast("Minimal transfer Rp 1.000."); if(a>w.balance)return toast("Saldo tidak cukup."); w.balance-=a; add("minus",a,"Transfer ke "+to); document.getElementById("transferTo").value=""; document.getElementById("transferAmount").value=""; persist(); toast("Transfer tercatat") }\n    if(e.target.id==="saveBtn"){ const a=Number(document.getElementById("saveAmount").value); if(!Number.isFinite(a)||a<1000)return toast("Minimal tabung Rp 1.000."); if(a>w.balance)return toast("Saldo tidak cukup."); w.balance-=a; w.savings+=a; add("minus",a,"Masuk Tabungan"); document.getElementById("saveAmount").value=""; persist(); toast("Berhasil ditabung") }\n    if(e.target.id==="withdrawSaveBtn"){ const a=Number(document.getElementById("withdrawSaveAmount").value); if(!Number.isFinite(a)||a<1000)return toast("Minimal Rp 1.000."); if(a>w.savings)return toast("Tabungan tidak cukup."); w.savings-=a; w.balance+=a; add("plus",a,"Ambil dari Tabungan"); document.getElementById("withdrawSaveAmount").value=""; persist(); toast("Dana dikembalikan ke saldo") }\n  })\n  render()\n})()\n