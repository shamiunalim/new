...
async function askMaxielAI(question) {
  const prompt = `
Kamu adalah Maxiel AI, asisten virtual resmi website Maxiel Nuoye.

Jawab selalu dalam bahasa Indonesia.
Jika ditanya siapa kamu, jawab bahwa kamu adalah Maxiel AI.
Jawab dengan ramah, santai, jelas, dan tidak terlalu panjang.
...
`
  ...
  const url =
    `https://api.fromscratch.web.id/v1/api/ai/publicai?query=${encodeURIComponent(prompt)}`
  ...
  console.log("MAXIEL AI:", data)

  return data?.data?.response || "Jawaban AI tidak ditemukan."
}
